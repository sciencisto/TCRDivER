...
def calculate_Zpi_parallel_random(lambdas_list, frequencies, number_of_clones):
    # random assign distances as a distance list. 
    distance_list_cdr3_i = np.random.random(size=number_of_clones)
...
def make_diversity_w_similarity_dict_random(cdr3_freq_subsample_dict, cdr3_order_subsample_dict, clones, path_to_wdir, sample_name , list_of_qs, lambdas_list, cc_size):
    """
    z = np.array([vals])
    z = z**(1./(1.-q))
    ...
    for cdr3_i in clones:
        process = pool.apply_async(calculate_Zpi_parallel_random, args =(lambdas_list, frequencies, len(clones)))
        Que.append(process)
...
if __name__ == '__main__':
    ...
    freqs = np.random.random(size=len(clones))
    cdr3_freq_dict = {clone:freq for clone, freq in zip(clones, freqs)}
    cdr3_freq_dict = normalize(cdr3_freq_dict, target=1.0)
    if identity == 'ID':
        diversity_dict = make_diversity_w_identity_dict(cdr3_freq_dict, qs)
    #
    else:
        diversity_dict = make_diversity_w_similarity_dict_random(cdr3_freq_dict, cdr3_order_dict, clones, path_to_wdir, sample_name , qs, lambdas_list, cc_size)
